...
public class OverviewReport {
	...
	public static void makePieChart(int iTotalTestCases){
		...
		int ntPass = totaltest -(DriverScript.countTestNotExecuted +DriverScript.countTestFailed);
		indexData.append("['Passed',     "+ntPass+"],");
		indexData.append("['Failed',     "+DriverScript.countTestFailed+"],");
		indexData.append("['Not Executed',"+DriverScript.countTestNotExecuted+"],");
		indexData.append("]);\n");
	...
	private static void writeindexhtml() throws IOException {
		Writer out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(Constants.htmlOutPath+ "Conformiq Test Automation Execution Report.html"), "UTF-8"));
		try {
	...
	public static void generateGeneralInfo(int iTotalTestCases) {
		...
		indexData.append("<TD COLSPAN=3 CLASS=fieldvalue>"+Constants.projectName+"</TD>\n");
		...
		indexData.append(" <TD CLASS=fieldvalue> <B><FONT COLOR=\"BLACK\">"+DriverScript.countTestNotExecuted+"/"+totaltest+" ["+((DriverScript.countTestNotExecuted * 100)/totaltest)+"%]"+"</TD>\n");
		indexData.append("  </TR> <TR>\n");
		indexData.append(" <TD CLASS=projectfieldheader>Total Tests Failed</TD>\n");
		indexData.append("   <TD COLSPAN=3 CLASS=fieldvalue> <B><FONT COLOR=\"RED\">"+DriverScript.countTestFailed+"/"+totaltest+" ["+((DriverScript.countTestFailed * 100)/totaltest)+"%]"+"</TD>\n");
