...
/** Conformiq generated test case
	Test Case 19
*/
public class Test_Case_19 extends PageObjectBase
{
	...
	testReport.generateGeneralInfo(overallTestData, "Test_Case_19", "TC_Test_Case_19", "",ne);
	...
	getScreenshot(driver,Configurations.screenshotLocation , "Test_Case_19","Step_2");
	...
	getScreenshot(driver,Configurations.screenshotLocation , "Test_Case_19","Step_3");
	...
	getScreenshot(driver,Configurations.screenshotLocation , "Test_Case_19","Step_4");

	Reporter.log("Step - 5- Fill Signup form Register screen");

	testReport.fillTableStep("Step 5", "Fill Signup form Register screen");

	register_page_init.set_First_Name("");
	register_page_init.set_Last_Name("");
	register_page_init.set_Address("");
	register_page_init.set_City("");
	register_page_init.set_State("");
	register_page_init.set_Zip_Code("0");
	register_page_init.set_Phone("0");
	register_page_init.set_SSN("");
	register_page_init.set_Username("");
	register_page_init.set_Password("");
	register_page_init.set_Confirm("");
	register_page_init.set_Dob("[Don't Care]");
	getScreenshot(driver,Configurations.screenshotLocation , "Test_Case_19","Step_5");
	...
	getScreenshot(driver,Configurations.screenshotLocation , "Test_Case_19","Step_6");

	Reporter.log("Step - 7- Verify Reject Register Action");

	testReport.fillTableStep("Step 7", "Verify Reject Register Action");

	reject_register_page_init.field_First_Name("First name is required");
	reject_register_page_init.field_Last_Name("Last name is required");
	reject_register_page_init.field_Adress("Address is required");
	reject_register_page_init.field_City(" City is required");
	reject_register_page_init.field_State("State is required");
	reject_register_page_init.field_Zip_Code("Zip code is required");
	reject_register_page_init.field_Phone("Phone number is required");
	reject_register_page_init.field_SSN("SSN is required");
	reject_register_page_init.field_UserName("User name is required");
	reject_register_page_init.field_Password("Password is required");
	reject_register_page_init.field_Confirm("Password Conformation is required");
	}
	@AfterTest
	public void export(){
		...
		driver.close();
		try {
			testReport.writeTestReporthtml(overallTestData, "Test_Case_19");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
